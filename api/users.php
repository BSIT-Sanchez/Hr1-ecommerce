<?php
// users.php - RESTful API for Users (no email)
...
if ($method === 'POST') {
    ...
    $role = sanitize($conn, $input['role'] ?? 'applicant'); // Default role: Applicant
    ...
    if ($stmt->execute()) {
        echo json_encode(['status' => 'success', 'message' => 'User created', 'role' => $role]);
    } else {
